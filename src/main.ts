...
import {Tensor3} from './base.js';
import {makeNoise2D} from '../lib/open-simplex-2d.js';
import {HeightmapResult, heightmap, noise} from './worldgen.js';
...
const checkReady = () => {
  ...
  const seed = int(1717);
  const noiseJS = makeNoise2D(seed);
  const noiseWasm = module.asm.createNoise2D(seed);

  const x = -17.3;
  const y = -34.6;
  console.log('Testing OpenSimplex2D noise:');
  console.log('    JS:', noiseJS(x, y));
  console.log('  WASM:', module.asm.queryNoise2D(noiseWasm, x, y));

  console.log('');
  console.log('Testing MGv7 noise:');
  console.log('    JS:', noise(int(x), int(y)));
  console.log('  WASM:', module.asm.noise(int(x), int(y)));
  ...
  const show_js = (x: HeightmapResult) => {
    const {height, snow_depth, tile} = x;
    return `${height} / ${snow_depth} / ${tile}`;
  };
  const show_wasm = (x: WasmHeightmap) => {
    const p = x >> 2;
    return `${heap[p + 1]} / ${heap[p + 2]} / ${heap[p]}`;
  };

  console.log('');
  console.log('Testing heightmap:');
  console.log('    JS:', show_js(heightmap(int(x), int(y), kTestBlocks)));
  console.log('  WASM:', show_wasm(module.asm.heightmap(int(x), int(y))));

  console.log('');
  console.log('Testing loadChunk:');
  const [cx, cz] = [int(0), int(0)];

  const HeapU8 = module.HEAPU8;
  let wasm_raw = module.asm.loadChunk(cx, cz);

  const column = new Column();
  const js_loadChunk = loadChunk(kTestBlocks);
  for (let z = 0; z < 16; z++) {
    for (let x = 0; x < 16; x++) {
      column.clear();
      js_loadChunk(int(x + cx * 16), int(z + cz * 16), column);
      const data = [];
      for (let i = int(0); i < column.getSize(); i++) {
        data.push(column.getNthBlock(i, kEmptyBlock));
        data.push(column.getNthLevel(i));
      }
      data.push(kEmptyBlock);
      data.push(0xff);
      const decorations: [int, int][] = [];
      for (let i = 0; i < (column as any).decorations.length; i += 2) {
        const block = (column as any).decorations[i + 0];
        const index = (column as any).decorations[i + 1];
        let found = false;
        for (const pair of decorations) {
          if (pair[1] !== index) continue;
          pair[0] = block;
          found = true;
          break;
        }
        if (!found) {
          decorations.push([block, index]);
        }
      }
      data.push(decorations.length);
      for (const pair of decorations) {
        data.push(pair[0]);
        data.push(pair[1]);
      }
      const js_data = data.join('/');

      let wasm_end = wasm_raw + 2;
      while (HeapU8[wasm_end - 1] !== 0xff) wasm_end += 2;
      const num_decorations = HeapU8[wasm_end];
      wasm_end += 2 * num_decorations + 1;
      const wasm_data = HeapU8.subarray(wasm_raw, wasm_end).join('/');
      wasm_raw = wasm_end as WasmCharPtr;

      //console.log('    JS:', js_data);
      //console.log('  WASM:', wasm_data);

      assert(js_data === wasm_data);
    }
  }
  console.log(`loadChunk matched at chunk: (${cx}, ${cz})`);
  ...
  for (let a = 0; a < 10; a++) {
    ...
    for (let i = 0; i < iterations; i++) {
      for (let z = 0; z < 16; z++) {
        for (let x = 0; x < 16; x++) {
          column.clear();
          js_loadChunk(int(x + (cx + (i & 1)) * 16), int(z + cz * 16), column);
        }
      }
    }
    console.log('    JS time:', performance.now() - start_js);
    const start_wasm = performance.now();
    for (let i = 0; i < iterations; i++) {
      module.asm.loadChunk(int(cx + (i & 1)), cz);
    }
    console.log('  WASM time:', performance.now() - start_wasm);
  }

  console.log('');
  const voxels = new Tensor3(16, 256 as int, 16);
  const fakeChunk = {
    setColumn(x: int, z: int, start: int, count: int, block: BlockId) {
      const index = voxels.index(x, start, z);
  ...
  for (let x = -12; x <= 12; x++) {
    for (let z = -12; z <= 12; z++) {
      if (x * x + z * z >= 12.5 * 12.5) continue;
      for (let a = int(0); a < 16; a++) {
        for (let b = int(0); b < 16; b++) {
          column.clear();
          js_loadChunk(int(a + 16 * x), int(b + 16 * z), column);
          column.fillChunk(a, b, fakeChunk as any, false);
        }
      }
    }
  }
  console.log('  JS updateWorld time:', performance.now() - start_js);
  module.asm.initializeWorld(12);
  const start_wasm = performance.now();
  for (let a = 0; a < 500; a++) {
    module.asm.updateWorld(0, 0);
  }
  console.log('WASM updateWorld time:', performance.now() - start_wasm)
  ...
  console.log('Testing perf:');
  for (let a = 0; a < 10; a++) {
    const iterations = 10, range = 16;
    const start_js = performance.now();
    for (let i = 0; i < iterations; i++) {
      for (let x = -range; x < range; x++) {
        for (let z = -range; z < range; z++) {
          //noise(int(x), int(z));
          heightmap(int(x), int(y), kTestBlocks);
        }
      }
    }
    console.log('    JS time:', performance.now() - start_js);
    ...
    for (let i = 0; i < iterations; i++) {
      for (let x = -range; x < range; x++) {
        for (let z = -range; z < range; z++) {
          //module.asm.noise(int(x), int(y));
          module.asm.heightmap(int(x), int(y));
        }
      }
    }
    console.log('  WASM time:', performance.now() - start_wasm);
...
(window as any).beforeWasmCompile = (env: any) => {
  env.js_loadChunk = (x: int, z: int) => console.log(`loadChunk(${x}, ${z})`);
};
(window as any).onWasmCompile = (m: WasmModule) => { module = m; checkReady(); };
